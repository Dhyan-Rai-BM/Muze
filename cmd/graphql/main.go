...
import (
	...
	pb "github.com/dhyanrai/muze/proto"
)
...
<body>
    <div class="container">
        ...
        <div class="section">
            <h3>JWT Token for Authentication</h3>
            <div class="jwt-token">%s</div>
            <p><strong>Use this token in the Authorization header:</strong> <code>Bearer [token]</code></p>
        </div>
        ...
        <div class="section">
            ...
            <h4>Get Posts</h4>
            <div class="code-block">curl -X POST http://localhost:7002/query \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer %s" \\
  -d '{"query": "query { getPosts(limit: 5) { posts { id content author { name } likes timestamp } } }"}'</div>

            <h4>Create Post</h4>
            <div class="code-block">curl -X POST http://localhost:7002/query \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer %s" \\
  -d '{"query": "mutation { createPost(content: \"Hello Muze!\") { id content author { name } likes } }"}'</div>

            <h4>Like Post</h4>
            <div class="code-block">curl -X POST http://localhost:7002/query \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer %s" \\
  -d '{"query": "mutation { likePost(id: \"1\") { id likes } }"}'</div>
        </div>
        ...
        <div class="note">
            <strong>Note:</strong> All mutations require the JWT token in the Authorization header. 
            The token above is automatically generated for testing purposes.
        </div>
    </div>
</body>
</html>
...
func executeGraphQLQuery(ctx context.Context, grpcClient pb.PostServiceClient, query string, variables map[string]interface{}) (map[string]interface{}, error) {
	// Simple GraphQL query parsing and execution
	...
	if strings.Contains(query, "likePost") {
		...
		if postID == "" {
			...
			if strings.Contains(query, "id:") {
				start := strings.Index(query, "id:") + 3
				end := strings.Index(query[start:], "\"")
				if end != -1 {
		...
		resp, err := grpcClient.LikePost(ctx, &pb.LikePostRequest{Id: postID})
		if err != nil {
...
func convertProtoPostToGraphQL(post *pb.Post) map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"author":    map[string]interface{}{"name": post.Author},
		"likes":     post.Likes,
